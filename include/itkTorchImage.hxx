...
constexpr unsigned int TorchImage<TPixel, VImageDimension>::ImageDimension;

template <typename TPixel, unsigned int VImageDimension>
constexpr torch::ScalarType TorchImage<TPixel, VImageDimension>::TorchValueType;

template <typename TPixel, unsigned int VImageDimension>
constexpr unsigned int TorchImage<TPixel, VImageDimension>::PixelDimension;

template <typename TPixel, unsigned int VImageDimension>
constexpr unsigned int TorchImage<TPixel, VImageDimension>::TorchDimension;

template <typename TPixel, unsigned int VImageDimension>
bool TorchImage<TPixel, VImageDimension>::SetDevice(
    typename Self::DeviceType deviceType) {
  switch (deviceType) {
  case Self::itkCUDA:
...
template <typename TPixel, unsigned int VImageDimension>
bool TorchImage<TPixel, VImageDimension>::SetDevice(
    typename Self::DeviceType deviceType, uint64_t cudaDeviceNumber) {
  switch (deviceType) {
  case Self::itkCUDA:
    if (m_DeviceType == Self::itkCUDA &&
        m_CudaDeviceNumber == cudaDeviceNumber) {
      return true; // no change
    }
    if (!(torch::cuda::is_available() &&
          cudaDeviceNumber < torch::cuda::device_count())) {
      return false;
    }
    if (m_Allocated) {
...
template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::GetDevice(
    typename Self::DeviceType &deviceType, uint64_t &cudaDeviceNumber) {
...
template <typename TPixel, unsigned int VImageDimension>
std::vector<int64_t>
TorchImage<TPixel, VImageDimension>::ComputeTorchSize() const {
...
template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::Allocate(
    typename Self::TensorInitializer tensorInitializer) {
  // itkImage does not call Superclass::Allocate.  Should we?
  // Superclass::Allocate( initializePixels );

  // Non-scalar pixel types are represented as additional dimensions in the
  // torch image.
  const std::vector<int64_t> torchSize = this->ComputeTorchSize();

  // Set up Tensor options
  c10::TensorOptions tensorOptions = torch::dtype(Self::TorchValueType)
...
template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::Initialize() {
  //
...
template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::FillBufferPart(
    int CurrentDimensions, const typename Self::SizeType &BufferSize,
  ...
  if (CurrentDimensions == 0) {
    ...
    for (typename Self::SizeValueType i = 0;
         i < BufferSize[CurrentDimensions - 1]; ++i) {
      TorchIndex.push_back(static_cast<int64_t>(i));
      this->FillBufferPart(CurrentDimensions - 1, BufferSize, TorchIndex,
                           value);
      TorchIndex.pop_back();
    }
  }
...
template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::SetPixel(
    const typename Self::IndexType &index,
    const typename Self::PixelType &value) {
...
TorchImage<TPixel, VImageDimension>::GetPixel(
    const typename Self::IndexType &index) {
  std::vector<torch::indexing::TensorIndex> TorchIndex;
  for (unsigned int i = 0; i < Self::ImageDimension; ++i) {
    TorchIndex.push_back(
        static_cast<int64_t>(index[Self::ImageDimension - 1 - i]));
  }
...
TorchImage<TPixel, VImageDimension>::GetBufferPointer() {
  return reinterpret_cast<typename Self::PixelType *>(
      m_Tensor.data_ptr<DeepScalarType>());
}
...
TorchImage<TPixel, VImageDimension>::GetBufferPointer() const {
  return reinterpret_cast<const typename Self::PixelType *>(
      m_Tensor.data_ptr<DeepScalarType>());
}

template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::Graft(const Self *data) {
  ...
  if (m_Allocated) {
    m_Tensor =
        torch::from_blob(data->m_Tensor.data_ptr(), data->m_Tensor.sizes(),
                         data->m_Tensor.options());
  }
}

template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::Graft(const DataObject *data) {
  if (data) {
...
template <typename TPixel, unsigned int VImageDimension>
TorchImage<TPixel, VImageDimension>::TorchImage() {
...
template <typename TPixel, unsigned int VImageDimension>
void TorchImage<TPixel, VImageDimension>::PrintSelf(std::ostream &os,
                                                    Indent indent) const {
  Superclass::PrintSelf(os, indent);
  os << indent << "m_DeviceType: " << m_DeviceType << std::endl

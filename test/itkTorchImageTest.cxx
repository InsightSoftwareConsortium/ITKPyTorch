...
int itkTorchImageTestByTypeAndDimension(const int SizePerDimension,
  ...
  // Choose ImageType::itkCUDA as the device type
  typename ImageType::DeviceType MyDeviceType = ImageType::itkCUDA;
  bool response = image->SetDevice(MyDeviceType);
  if (!response) {
    // ImageType::itkCUDA not available.  Trying ImageType::itkCPU instead.
  ...
  itkAssertOrThrowMacro(deviceType == MyDeviceType,
                        StructName + "::GetDevice failed for deviceType");
  itkAssertOrThrowMacro(cudaDeviceNumber == 0,
                        StructName + "::GetDevice failed for cudaDeviceNumber");
  ...
  itkAssertOrThrowMacro(pixelValue == firstValue,
                        StructName + "::FillBuffer failed");
  ...
  pixelValue = image->GetPixel(location0);
  itkAssertOrThrowMacro(pixelValue == secondValue,
                        StructName + "::GetPixel as lvalue failed");

  pixelValue = image->GetPixel(location1);
  itkAssertOrThrowMacro(pixelValue == firstValue,
                        StructName + "::GetPixel has side effect");

  image->SetPixel(location1, thirdValue);
  pixelValue = image->GetPixel(location1);
  itkAssertOrThrowMacro(pixelValue == thirdValue,
                        StructName + "::SetPixel failed");
  pixelValue = image->GetPixel(location0);
  itkAssertOrThrowMacro(pixelValue == secondValue,
                        StructName + "::SetPixel has side effect");
...
int itkTorchImageTest(int argc, char *argv[]) {
  ...
  // though we do not support 16-bit floats.
  {
    ...
    const int response =
        itkTorchImageTestByTypeAndDimension<PixelType, ImageDimension>(
            SizePerDimension, StructName, firstValue, secondValue, thirdValue);
    if (response != EXIT_SUCCESS) {
      return response;
    }

    // Run ITK_EXERCISE_BASIC_OBJECT_METHODS just once, rather than for every
    // pixel type.
    using ImageType = itk::TorchImage<PixelType, ImageDimension>;
    typename ImageType::Pointer image = ImageType::New();
  ...
  }
  {
    ...
    const typename PixelType::ValueType secondValue[VectorDimension] = {-100,
                                                                        32000};
    const typename PixelType::ValueType thirdValue[VectorDimension] = {100,
                                                                       -32000};
    const int response =
        itkTorchImageTestByTypeAndDimension<PixelType, ImageDimension>(
            SizePerDimension, StructName, firstValue, secondValue, thirdValue);
...

  {
    ...
    const typename PixelType::ValueType secondValue[VectorDimension] = {
        -310, 3100, -31000};
    const typename PixelType::ValueType thirdValue[VectorDimension] = {
        310, -3100, 31000};
    const int response =
        itkTorchImageTestByTypeAndDimension<PixelType, ImageDimension>(
            SizePerDimension, StructName, firstValue, secondValue, thirdValue);
  ...
  }
  {
    ...
    constexpr int ImageDimension = 5;
    const std::string StructName =
        "TorchImage< CovariantVector< int16_t, 4 >, 5 >";
  ...
  }
  {
    ...
    using PixelType =
        itk::Vector<itk::Vector<itk::RGBPixel<uint8_t>, VectorDimension1>,
                    VectorDimension2>;
    constexpr int ImageDimension = 4;
    const std::string StructName =
        "TorchImage< Vector< Vector< RGBAPixel< uint8_t >, 2 >, 3 >, 4 >";
    const int SizePerDimension = 3;
    const uint8_t firstValue0[] = {1, 1, 1};
    const uint8_t secondValue0[] = {4, 64, 255};
    const uint8_t thirdValue0[] = {0, 128, 1};
    const itk::RGBPixel<uint8_t> firstValue1[VectorDimension1] = {firstValue0,
                                                                  secondValue0};
    const itk::RGBPixel<uint8_t> secondValue1[VectorDimension1] = {secondValue0,
                                                                   thirdValue0};
    const itk::RGBPixel<uint8_t> thirdValue1[VectorDimension1] = {thirdValue0,
                                                                  firstValue0};
    const itk::Vector<itk::RGBPixel<uint8_t>, VectorDimension1>
        firstValue[VectorDimension2] = {firstValue1, secondValue1, firstValue1};
    const itk::Vector<itk::RGBPixel<uint8_t>, VectorDimension1>
        secondValue[VectorDimension2] = {thirdValue1, thirdValue1, thirdValue1};
    const itk::Vector<itk::RGBPixel<uint8_t>, VectorDimension1>
        thirdValue[VectorDimension2] = {secondValue1, firstValue1,
                                        secondValue1};
    const int response =
        itkTorchImageTestByTypeAndDimension<PixelType, ImageDimension>(
            SizePerDimension, StructName, firstValue, secondValue, thirdValue);
